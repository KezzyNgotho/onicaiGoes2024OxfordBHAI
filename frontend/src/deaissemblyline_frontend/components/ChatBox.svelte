<script lang="ts">
  ...
  import Message from './AiCreationProcess/Steps/ChatUserInterface/Message.svelte';
...
// Toggle whether user wants their messages to be stored
  ...
  const generateProgressCallback = (_step: number, message: string) => {
    replyText = message;
    messages = [...messages.slice(0, -1), { sender: 'AI Assistant', content: replyText }];
  };
  ...
  async function sendMessage() {
    ...
    if(newMessageText.trim() !== '') {
      ...
      newMessageText = '';
      try {
        messages = [...messages, { sender: 'AI Assistant', content: replyText }];
        const reply = await modelCallbackFunction(newPrompt, generateProgressCallback);
        messages = [...messages.slice(0, -1), { sender: 'AI Assistant', content: reply }]; 
      } catch (error) {
        console.error("Error getting response from model: ", error);
        messages = [...messages, { sender: 'AI Assistant', content: "There was an error unfortunately. Please try again." }];
      }
      replyText = 'Thinking...';
    }
